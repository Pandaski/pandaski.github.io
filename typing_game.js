...
G.F.loadMain = function () { 
    ...
    G.makeGob('helpDetail', G.O.viewport)
        ...
        .setSrc('这是一个结合了英语背诵和打字训练的小游戏，以下是使用方法。' + 
        '<ol ">' + 
        '<li>用户提前准备一个单词列表文本文件(txt文件)。文件里每行有一个英语单词和对应的中文词，用逗号（,）分开。如图所示。<br>'+
        '<img width="250px" src="data:image/png;base64,' + resources.image.EXAMPLE + '">' +
        '</li>' +
        '<li>英语单词里可以有字母或空格，但不能有其他符号。</li>' +
        '<li>点击右下角的“Load Words”加载单词到游戏中。</li>' +
        '<li>点击游戏中间的“Start”开始游戏。</li>' +
        '<li>游戏会打乱单词列表后逐一显示中文词语，并同时朗读英语单词发音（需要网络）。用户需要在键盘上正确的逐一打出该单词的每个字母（包括空格），直到全部正确输入完成。如果输入字母不正确，游戏会通过蜂鸣声提示。</li>' +
        '<li>请注意游戏是区分大小写的，例如遇到“Russia”时必须按Shift + r才能正确的输入。</li>' +
        '<li>当全部单词都测试完成，游戏会结束。这时可以选择用同一批单词重新游戏，或加载新一批单词。</li>' +
        '</ol>')
        .turnOff();
